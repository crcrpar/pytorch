...
namespace {
  ...
  static void adatptive_avg_pool3d_out_frame(
            scalar_t *input_p,
            scalar_t *output_p,
            int64_t sizeD,
            int64_t isizeT,
      ...
      IntArrayRef output_size)
  {
    ...
    if (input.ndimension() == 4)
    {
      ...
      AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "adaptive_avg_pool3d_cpu", [&] {
          auto input_data = input.data<scalar_t>();
          auto output_data = output.data<scalar_t>();
          adaptive_avg_pool3d_out_frame<scalar_t>(input_data, output_data,
                                                  sizeD,
    ...
    #pragma omp parallel for private(b)
      ...
      {
        AT_DISPATCH_FLOATING_TYPES(input.scalar_t(), "adaptive_avg_pool3d_cpu", [&] {
            auto input_data = input.data<scalar_t>();
            auto output_data = output.data<scalar_t>();
            adaptive_avg_pool3d_out_frame<scalar_t>(input_data + b * input.stride(0),
  ...
  #pragma omp parallel for private(d)
    for (d = 0; d < sizeD; d++)
    {
      scalar_t *gradInput_p_d = gradInput_p + d * iszieT * isizeW * isizeH;
      scalar_t *gradOutput_p_d = gradOutput_p_d + d * osizeT * osizeW * osizeH;

      /* calculate average */
      int64_t ot, oh, ow;
      for (ot = 0; oh < osizeT; ot++)
      {
        ...
        for (oh = 0; oh < osizeH; oh++)
        {
          ...
          for (ow = 0; ow < osizeW; ow++)
          {
            int istartW = start_index(iw, osizeW, isizeW);
            int iendW = end_index(iw, osizeW, isizeW);
            int kW = iendW - istartW;

            scalar_t grad_delta = gradOutput_p_d(ot * osizeH + oh * osizeW + ow) / kT / kH / kW;
            ...
            for (it = istartT; it < iendT; it++)
            {
              for (ih = istartH; ih < iendH; ih++)
              {
                ...
                {
                  /* update gradient */
                  gradInput_p_d[it * isizeH + ih * isizeW + iw] += grad_delta;
                }
              }
            }
          }
  ...
  Tensor adaptive_avg_pool3d_cpu(at::Tensor const& input, IntArrayRef output_size)
  {
  ...
  Tensor adaptive_avg_pool3d(at::Tensor const& input, IntArrayRef output_size) {
    if (output_size[0] == 1 && output_size[1] == 1 && output_size[2] == 1) {
      ...
      return input.ndimension() == 3 ? out.view({input.size(0), 1, 1, 1}) : out.view({input.size(0), input.size(1), 1, 1, 1});
    } else {
