...
template <typename func_t, typename inp_offset_calc_t_1, typename inp_offset_calc_t_2, typename out_offset_calc_t>
__global__ void distribution_binary_elementwise_kernel(
    int numel,
    func_t f,
    std::pair<uint64_t, uint64_t> seeds,
    typename ternary_function_traits<func_t>::result_type *output_data,
    const typename ternary_function_traits<func_t>::arg2_t *inp1_data,
    const typename ternary_function_traits<func_t>::arg3_t *inp2_data,
    inp_offset_calc_t_1 inp_calc_1,
    inp_offset_calc_t_2 inp_calc_2,
  ...
  input_t_1 inputs1[THREAD_WORK_SIZE];
  input_t_2 inputs2[THREAD_WORK_SIZE];
  ...
  for (int i = 0; i < THREAD_WORK_SIZE; i++) {
    ...
    auto offsets_1 = inp_calc_1.get(input_idx);
    inputs1[i] = inp1_data[offsets_1[0]];
    auto offsets_2 = inp_calc_2.get(input_idx);
    inputs2[i] = inp2_data[offsets_2[0]];
  ...
  for (int i = 0; i < THREAD_WORK_SIZE; i++) {
    ...
    output_data[offsets[0]] = f(state, inputs1[i], inputs2[i]);
    thread_idx += num_threads;
  }
}
...
void distribution_binary_kernel(TensorIterator &iter, std::pair<uint64_t, uint64_t> seeds, const func_t &f) {
  ...
  ouptut_t *output_data = static_cast<output_t *>(iter.data_ptr(0));
  ...
  uint64_t gid = (numel + block_work_size - 1) / block_work_size;
  auto stream = at::cuda::getCurrentCUDAStream();

  if (iter.is_contiguous()) {
    distribution_binary_elementwise_kernel<<<grid,num_threads, 0, stream>>>(
        numel, f, seeds, output_data, input_data_1, input_data_2,
        TrivialOffsetCalculator<1>(), TrivialOffsetCalculator<1>(), TrivialOffsetCalculator<1>());
  } else {
    ...
    distribution_binary_elementwise_kernel<<<grid, num_threads, 0, stream>>>(
        numel, f, seeds, output_data, input_data_1, input_data_2,
        make_input_offset_calculator<1>(iter), make_input_offset_calculator<2>(iter),
        make_output_offset_calculator(iter));
  }
}
