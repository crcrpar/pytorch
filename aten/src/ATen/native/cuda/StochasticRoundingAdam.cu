...
__global__ void stochastic_rounding_adam_step_kernel(
  ...
  for  (int i = blockIdx.x * blockDim.x + threadIdx.x; i < numel; i += blockDim.x * gridDim.x) {
    ...
    weights.data[w_offset] = round_stochastically<scalar_t>(weight, random_values.x);
    exp_avg.data[m_offset] = round_stochastically<scalar_t>(m, random_values.y);
    exp_avg_sq.data[v_offset] = round_stochastically<scalar_t>(sqrtf(v), random_values.z);
    if (is_amsgrad) {
      max_exp_avg_sq.data[max_v_offset] = round_stochastically<scalar_t>(sqrtf(max_v), random_values.w);
    }
  }
}
